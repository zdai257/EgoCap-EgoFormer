...
class PositionEmbeddingVideoSine(nn.Module):
    ...
    def forward(self, mask, x_device):

        assert mask is not None
        not_mask = ~mask
        y_embed = not_mask.cumsum(1, dtype=torch.float32)
        x_embed = not_mask.cumsum(2, dtype=torch.float32)
        t_embed = not_mask.cumsum(axis=3, dtype=torch.float32)
        if self.normalize:
            eps = 1e-6
            y_embed = y_embed / (y_embed[:, -1:, :, :] + eps) * self.scale
            x_embed = x_embed / (x_embed[:, :, -1:, :] + eps) * self.scale
        ...
        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=x_device)
        dim_t = self.temperature ** (2 * (dim_t // 2) / self.num_pos_feats)
        ...
        pos = torch.cat((pos_y, pos_x, pos_t), dim=4).permute(0, 4, 1, 2, 3)
        #print(pos.shape)
        return pos
