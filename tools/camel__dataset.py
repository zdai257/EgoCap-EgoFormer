...
# Customised dataloader for EgoCap
class EgoCap(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
                 cut_validation=False):
        ...
        roots = {}
        roots['train'] = {
            'img': os.path.join(img_root, 'static'),
            'cap': os.path.join(ann_root, 'doc', 'EgoCap_annotations_ref.json')
        }
        roots['val'] = {
            'img': os.path.join(img_root, 'static'),
            'cap': os.path.join(ann_root, 'doc', 'EgoCap_annotations_ref.json')
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids)
        examples = self.train_examples + self.val_examples + self.test_examples
        super(EgoCap, self).__init__(examples, image_field, text_field)
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        with open(roots['train']['cap'], 'r') as f:
            egocap_ann = json.load(f)

        for key, val in egocap_ann.items():

            filename = os.path.join('Split' + str(val['SplitIndex']).zfill(2), key)

            for caption in val['captions']:
                example = Example.fromdict({'image': os.path.join(roots['train']['img'], filename), 'text': caption})

                if val['SplitIndex'] in [4, 5, 6, 7, 8, 9, 11, 12, 13, 14, 15, 16, 18, 19, 20, 21]:
                    train_samples.append(example)
                elif val['SplitIndex'] in [1, 2]:
                    val_samples.append(example)
                elif val['SplitIndex'] in [3, 10, 17]:
                    test_samples.append(example)
